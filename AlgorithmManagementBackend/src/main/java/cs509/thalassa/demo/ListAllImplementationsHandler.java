 ...
 */
public class ListAllImplementationsHandler implements RequestHandler<ListImplementationRequest,AllImplementationsResponse> {
	 ...
	 * @throws Exception 
	 */
	List<Implementation> getImplementations(String parentAlgorithm) throws Exception {
		logger.log("in getImplementations");
		ImplementationDAO dao = new ImplementationDAO(logger);
		
		return dao.getRelatedImplementations(parentAlgorithm);
	}
	...
	**/
	...
	@Override
	public AllImplementationsResponse handleRequest(ListImplementationRequest req, Context context)  {
		logger = context.getLogger();
		logger.log("Loading Java Lambda handler to list all implementations");
		logger.log("ParentAlgorithmId: " + req.parentAlgorithm);
		//logger.log("ImplementationId: " + req.);
		AllImplementationsResponse response;
		try {
			// get all user defined constants AND system-defined constants.
			// Note that user defined constants override system-defined constants.
			List<Implementation> list = getImplementations(req.parentAlgorithm);
			/**
			...
			**/
			response = new AllImplementationsResponse(list, 200);
		} catch (Exception e) {
			response = new AllImplementationsResponse(403, e.getMessage());
		}
